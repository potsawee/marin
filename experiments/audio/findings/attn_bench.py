...
def materialized(q, kk, v):   # what VANILLA/reference does: O(S^2) scores
    qs = q.transpose(0,2,1,3); ks = kk.transpose(0,2,1,3); vs = v.transpose(0,2,1,3)
...
for name, fn in [("materialized", materialized), ("dpa-xla", dpa("xla")), ("dpa-cudnn", dpa("cudnn"))]:
    try:
        g = jax.jit(fn)
        o = g(q, kk, v); o.block_until_ready()
        ts=[]
        for _ in range(10):
            t=time.time(); o=g(q,kk,v); o.block_until_ready(); ts.append(time.time()-t)
