...
fwd_per_tok = arm_f_fwd_per_token(cfg)
PEAK = 362.05e12
...
def step(m, ex):
    def loss(m):
        return m.compute_next_token_loss(ex, key=None).scalar()
...
for bs in (2, 8, 32):
    ...
    dt = min(ts)
    flops = 3 * fwd_per_tok * bs * 4096
    print(
        f"  batch={bs:2d} (1 GPU): {dt*1000:7.1f} ms/step  "
        f"MFU={flops/(PEAK*dt)*100:5.1f}%  ({bs*4096/dt/1e3:.0f}k tok/s)"
    )
